...
// fetch
// onload = initialLoad
// breedSelect.onchange = handleBreedChange

// axios
onload = axiosInitialLoad
breedSelect.onchange = axiosHandleBreedChange


const getFavoritesBtn = document.getElementById("getFavoritesBtn");
getFavoritesBtn.onclick = populateFavorites
...
async function getFavorites() {
    const { data: favorites } = await axios.get(API_FAV_URL)
    return favorites
}

async function populateFavorites() {
    const favorites = await getFavorites()
    breedSelect.selectedIndex='default'

    Carousel.clear()

     // populate the carousel with cat images
     for (const favorite of favorites) {
        const item = Carousel.createCarouselItem(favorite.image.url, `Picture of a cat`, favorite.image.id)
        Carousel.appendCarousel(item)
    }
...
export async function deleteFavorite(favoriteId) {
    axios.delete(`${API_FAV_URL}/${favoriteId}`)
        .then(() => {
            const favBtn = getFaveBtn(`[data-fav-id='${favoriteId}']`)
...
export async function setFavorite(imgId) {
    const response = await axios.post(API_FAV_URL, { 'image_id': imgId, 'sub_id': '508' })
    const { message, id: favId } = response.data

    if (message === 'SUCCESS') {
        const favBtn = getFaveBtn(`[data-img-id='${imgId}']`)
        favBtn.dataset.favId = favId
        favBtn.classList.add('favorite')
    }
}

function getFaveBtn(selector) {
